...
def make_neural_net_no_poly(n_input, hidden_layer_sizes, keep_prob_tf):
    if len(hidden_layer_sizes) < 1:
        raise ValueError('hidden_layer_sizes must contain at least one integer')
    ...
    return params, major_inputs, minor_inputs, logprobs_major
...
def get_ll_from_nn_no_poly(logprobs_major, counts):
    tmp = tf.multiply(logprobs_major, counts)
    ll = tf.reduce_sum(tmp, axis = [0,1])
    return ll
...
def get_ll_and_grads_no_poly_tf(n_input, hidden_layer_sizes):
    keep_prob_tf = tf.placeholder(tf.float64, [])
    params, major_inputs, minor_inputs, lpM = make_neural_net_no_poly(n_input, hidden_layer_sizes, keep_prob_tf)
    counts = tf.placeholder(tf.float64, [None, 4])
    ll = get_ll_from_nn_no_poly(lpM, counts)

    grads = tf.gradients(ll, params)
    return params, major_inputs, minor_inputs, counts, ll, grads, keep_prob_tf
...
def loglike_and_gradient_wrapper_no_poly(params, cm, lo, maj, mino, keep_prob, ll_aux, sess):
    '''
    params are current parameter values
    cm is the localcm
    lo is tuple of tuple of direction-locobs
    ...
    # Expand ll_aux
    params_tf, major_inputs, minor_inputs, counts_tf, ll_tf, grads_tf, keep_prob_tf = ll_aux
    ...
    if mino == 'N':
        ...
        lls = []
        for alt_minor in alt_minors:
            tll, tgrad = loglike_and_gradient_wrapper_no_poly(params, cm, lo, maj, alt_minor, keep_prob, ll_aux, sess)
            lls.append(tll)
            gradients.append(tgrad)
        lls = np.array(lls)
        gradients = np.array(gradients)
...
def main():
    ...
    #fin = h5py.File('sims_TR3_context_2_rb_20_localcm_more_heterplasmy.h5', 'r')
    fin = h5py.File('TR21_context_2_rb_20_localcm_small.h5', 'r')
    ...
    #keep_prob = 0.7  # keep at 1.0 for comparing loglikes in different 'runs'
    keep_prob = 1.0
    ...
    if poly:
        ...
        ll_aux = get_ll_and_grads_no_poly_tf(num_inputs, hidden_layer_sizes)
        total_num_params = int(ll_aux[0].shape[0])
        nn_par_vals = npr.normal(size = total_num_params, scale = np.sqrt(2.0/num_inputs))
        freqs = bws.get_freqs(num_f)
        windows = bws.get_window_boundaries(num_f)
        par_vals = nn_par_vals.astype(np.float64)
        init = tf.global_variables_initializer()

        sess = tf.Session()
        sess.run(init)
        ll1, grads = loglike_and_gradient_wrapper_no_poly(par_vals, cm, lo, maj, mino, keep_prob, ll_aux, sess)
        print('ll', ll1)
        sess.close()
        ...
        sess = tf.Session()
        sess.run(init)
        ll2, grads = loglike_and_gradient_wrapper(par_vals, cm, lo, maj, mino, 3, lf, l1mf, freqs, windows, keep_prob, ll_aux, sess)
        print(ll2)
        sess.close()
