...
# TODO replace with nn.get_num_params, after creating
...
remaining_args = [pool, hidden_layer_sizes, num_pf_params, freqs, windows]

def grad_target(params, batch, pool, hidden_layer_sizes, num_pf_params, freqs, windows):
    loc_gradient_args = []
    for key, major, minor in batch:
        ...
        lo = [[lo['f1'][:], lo['f2'][:]], [lo['r1'][:], lo['r2'][:]]]
        num_obs = cm.shape[0]
        loc_gradient_args.append([params, cm, lo, num_obs, major, minor, hidden_layer_sizes, num_pf_params, freqs, windows])
    lls_and_grads = pool.map(nn.loglike_and_gradient_arg_wrapper, loc_gradient_args)
    loc_lls, loc_grads = zip(*lls_and_grads)
    grad = np.sum(loc_grads, axis = 0)
    return -1.0*grad
